...
echo "🤖 Applying Vire V5 Agentic Patch (LLM prompt → copy+layout → Gutenberg export)"

ROOT_DIR="$(cd "$(dirname "$0")" && pwd)"
V5="$ROOT_DIR/vire-agent/v5"
AG="$V5/agentic"

mkdir -p "$AG"

# ---------------------------------------------------------
# 1) Layout Library (deterministic templates the LLM selects)
# ---------------------------------------------------------
cat <<'JSON' > "$AG/layout_library.json"
{
  "layouts": {
    "umbrella_home": {
      "sections": ["hero", "ecosystem_bento", "founders_note", "waitlist", "faq", "footer"]
    },
    "product_teaser": {
      "sections": ["hero", "problem_solution", "features_grid", "how_it_works", "founders_note", "waitlist", "footer"]
    },
    "enterprise_b2b": {
      "sections": ["hero", "proof_points", "security_compliance", "case_study", "pricing", "founders_note", "cta", "footer"]
    }
  },
  "section_blocks": {
    "hero": {
      "pattern": "hero",
      "blocks": ["headline", "subhead", "badges", "primary_cta", "secondary_cta", "neural_canvas_hint"]
    },
    "ecosystem_bento": {
      "pattern": "bento",
      "blocks": ["product_cards_6", "priority_badges", "coming_soon_badges"]
    },
    "problem_solution": {
      "pattern": "split",
      "blocks": ["problem", "solution", "why_now"]
    },
    "features_grid": {
      "pattern": "grid",
      "blocks": ["feature_6"]
    ...
    "proof_points": {
      "pattern": "stats",
      "blocks": ["metrics_6", "logos_strip"]
    },
    "security_compliance": {
      "pattern": "cards",
      "blocks": ["security_points", "ymyl_disclaimer_optional"]
    },
    "case_study": {
      "pattern": "story",
      "blocks": ["mini_case_study"]
    },
    "pricing": {
      "pattern": "pricing",
      "blocks": ["tiers_3", "fineprint"]
    },
    "faq": {
      "pattern": "accordion",
      "blocks": ["faq_6"]
    ...
    "founders_note": {
      "pattern": "eeat",
      "blocks": ["founders_note"]
    },
    "footer": {
      "pattern": "footer",
      "blocks": ["links", "disclaimer_optional"]
    }
  }
}
JSON
...
# ---------------------------------------------------------
# 2) LLM Site Generator (prompt → site.json + Gutenberg pages)
#    - NO wordpress dependency
#    - writes:
#        vire-agent/v5/outputs/site.json
#        vire-agent/v5/outputs/pages/*.html
# ---------------------------------------------------------
cat <<'PY' > "$AG/llm_sitegen.py"
#!/usr/bin/env python3
...
PY
chmod +x "$AG/llm_sitegen.py"

# ---------------------------------------------------------
# 3) Agentic generator wrapper (one command)
# ---------------------------------------------------------
cat <<'SH' > "$AG/prompt_to_site.sh"
#!/usr/bin/env bash
set -euo pipefail
...
SH
chmod +x "$AG/prompt_to_site.sh"

# ---------------------------------------------------------
# 4) Agentic one-command runner (keeps same V5 outputs)
#    Uses existing V5 local/deploy/verify scripts if present.
# ---------------------------------------------------------
cat <<'SH' > "$V5/vire_v5_agentic_onecmd.sh"
#!/usr/bin/env bash
set -euo pipefail
...
SH
chmod +x "$V5/vire_v5_agentic_onecmd.sh"

echo "✅ Agentic V5 patch installed."
echo "Next:"
echo "  ./vire-agent/v5/vire_v5_agentic_onecmd.sh \"<prompt>\" \"<product>\" \"elite-neural-luxe\""
